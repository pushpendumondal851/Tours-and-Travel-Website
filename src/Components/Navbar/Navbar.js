...
const Navbar = () => {
  ...
  return (
    <div>
      <nav className="absolute top-7 right-12 p-4 space-x-8 font-semibold text-lg md:text-lg hidden md:flex">
        {["home", "about-us", "contact"].map((section, index) => (
          <Link
            key={index}
            to={`/${section}`}
            className="transition-transform duration-300 hover:text-yellow-800 transform hover:scale-110"
      ...
      {isOpen && (
        <div className="absolute top-16 right-8 bg-black bg-opacity-50 text-white rounded-lg shadow-lg py-2 px-4">
          {["home", "about-us", "contact"].map((section, index) => (
            <Link
              key={index}
              to={`/${section}`}
